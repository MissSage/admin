...
import layerVue from './components/LgsLayer.vue'
import maskVue from './components/Mask.vue'
const LgsLayer = function (Vue:any, globalOption = {
  msgtime: 1.5 // msg消失时间
}) {
  const LgsLayerConstructor = Vue.extend(layerVue)
  ...
  /**
    ...
    */
  self.open = function (options:any) {
    options = mergeJson(options, defOptions)
    const id = `LgsLayer_${new Date().getTime()}_${seed++}`
    options.id = id
    options.layer = self
    const instance = new LgsLayerConstructor({
      data: options
    })
    if (options.type === 2) {
  ...
  /**
    ...
    * @return {[type]}         [description]
    */
  self.alert = function (content:any, options:any, yes:any) {
  ...
  /**
    ...
    * @return {[type]}         [description]
    */
  self.confirm = function (content:any, options:any, yes:any, cancel:any) {
    switch (typeof (options)) {
      case 'function':
        cancel = yes
        yes = options
  ...
  /**
    ...
    * @return {[type]}         [description]
    */
  self.msg = function (content:any, options:any, end:any) {
    switch (typeof (options)) {
      case 'function':
        end = options
        options = {}
  ...
  // loading
  self.loading = function (icon?:any, options?:any) {
  ...
  /**
    ...
    * @return {[type]}         [description]
    */
  self.tips = function (content:any, follow:any, options:any) {
    options = options || {}
    options.type = 4
    options.content = content || ''
  ...
  /**
    * 获取信息框
    */
  self.prompt = function (options:any = {
    formType: 1,
    value: ''
  }, yes = '', cancel = '') {
  ...
  self.close = function (id:any) {
    ...
    if (oElm) {
      ...
      if (self.instancesVue[id].main.shade) {
        ...
        layerMask && document.body.removeChild(layerMask)
  ...
  self.full = function (id = '') {
    const dom:any = document.querySelector('#' + id + ' .lv-icon-max')
    dom && dom.click()
  }
  /**
    * 手动最小化
  ...
  self.min = function (id = '') {
    const dom:any = document.querySelector('#' + id + ' .lv-icon-mini')
    dom && dom.click()
  }
  /**
    * 手动最小化
  ...
  self.restore = function (id = '') {
    const dom:any = document.querySelector('#' + id + ' .lv-icon-huanyuan')
    dom && dom.click()
  }
  ...
  /**
    ...
    * @return {[type]}         [description]
    */
  function mergeJson (options:any, def:any) {
...
// module.exports = LgsLayer;
// const Layer = withInstallFunction(LgsLayer, '$layer')
export default LgsLayer
