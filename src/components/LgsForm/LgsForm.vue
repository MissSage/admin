        ...
        >
          ...
          <div v-else :class="{ 'form-item-extra': item.extra }">
            ...
            <lgs-uploader
              ...
              :file-info="formFields[item.field]"
              :url="item.url"
              :img="item.type === 'img' || item.columnType === 'img'"
              :excel="item.type === 'excel'"
              :file-types="item.fileTypes ? item.fileTypes : []"
    ...
    const initFormRules = (init) => {
      ...
      props.formRules.forEach((row, xIndex) => {
        ...
        if (_count > 0 && row.length - _count > 0) {
          const _cellSize = (12 - _size) / (row.length - _count)
          row.forEach((x) => {
            if (!x.colSize) {
              x.colSize = _cellSize
            }
  ...
  methods: {
    ...
    getRule (item, formFields) {
      ...
      if (item.validator && typeof item.validator === 'function') {
        return {
          validator: (rule, val, callback) => {
            ...
            const message = item.validator(rule, val)
      ...
      if (inputTypeArr.indexOf(item.type) !== -1) {
        ...
        const type = item.type === 'mail' ? 'email' : types[item.columnType]
        const _rule = {
          required: true,
          message: message,
          trigger: 'blur',
      ...
      if (item.type === 'cascader') {
        return {
          ...
          validator: (rule, val, callback) => {
            if (this.isReadonly(item)) return callback()
            // 用户自定义的方法，如果返回了值，直接显示返回的值，验证不通过
            const _arr = this.formFields[item.field]
            if (!_arr || !_arr.length) {
