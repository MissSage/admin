<template>
  ...
  >
    ...
    <template #footer>
      ...
      <el-button v-if="!config.disableSubmit" size="mini" type="primary" @click="submit"
        >保 存</el-button
      >
    </template>
  </el-dialog>
...
export default defineComponent({
  ...
  emits: ['refreshData'],
  setup(props: any, ctx) {
    const { $messageSuccess, $messageError, $format } = useGlobal()
    ...
    const submit = () => {
      ruleForm.value?.validate(async (valid: any) => {
        ...
        if (Object.values(aInfo).length > 0) {
          queryParams.additionalInfo = aInfo
        }
