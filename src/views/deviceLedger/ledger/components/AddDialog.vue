<template>
  ...
  > -->
    <div class="tableContainer">
      ...
      <!--表格-->
      <CardTable :config="cardTableConfig" class="tableHeight" @row-click="clcikRow" />
    </div>
    <template #footer>
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    //默认选择
    const initSelect = (deviceIds: string) => {
      if (deviceIds) {
    ...
    const initTableData = async () => {
      ...
      //有父设备id的时候添加父设备id
      if (props.fatherId) {
        chooseIds.value.worth.push(props.fatherId)
      }
