<template>
  <div class="info-container">
    ...
    <el-form ref="baseForm" :model="formData">
      <SLFlexGroup>
        <el-form-item class="ls-flex-item--4" label="故障次数：" prop="deviceNo">
          <span>{{ formData.total }} 次</span>
        </el-form-item>
        <el-form-item class="ls-flex-item--4" label="最近维修：" prop="deviceName">
          <span>{{ formData.executeTime }}</span>
        </el-form-item>

        <el-form-item label="" prop="deviceName" class="ls-flex-item--1" style="height: 250px">
          <div ref="pic" style="height: 100%; width: 48%; display: inline-block"></div>
...
//接口
import { getRDataById, getRPicData, getRLineData } from '@/api/ledger/ledger'
import type { ICTCardTable } from '@/common/types/repair/plan'
...
//引格式化
import { TASKTYPE } from '@/common/constans/ledger'
...
export default {
  ...
  setup(props, ctx) {
    ...
    const cardTableConfigFile = ref<ICTCardTable>({
      ...
      columns: [
        ...
        { prop: '', label: '任务类型', formatter: row => TASKTYPE[row.type] },
        {
          prop: '',
          label: '任务开启时间',
    ...
    const initForm = () => {
      if (props.currentId) {
        ...
        getRDataById(paramsObj).then(res => {
          console.log('res ', res)
    ...
    const formatPinData = (data: any) => {
      if (data.data.length <= 0) {
        ...
        data.data.forEach((item: any) => {
          let pitem = { value: item.count, name: item.type }
          PinData.value.label.push(item.type)
    ...
    const initPicCharts = (item: any) => {
      ...
      getRPicData(paramsObj).then((res: any) => {
        console.log('res', res)
        //    调用数据转移方式
        formatPinData(res)
 ...
 `
    ...
    const initLineCharts = (item: any) => {
      ...
      getRLineData(paramsObj).then(res => {
        console.log('resLine', res)
        //获取数据后格式化想要的数据
        formatLineData(res)
