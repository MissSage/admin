...
import { deleteMaintStandards, getMaintStandardTable } from '@/api/maintenance/standard'
import { QueryListParam } from '@/common/types/common'
export default defineComponent({
  components: { CardTable, CardSearch, AddDialog },
  setup() {
    const { $messageSuccess, $messageError, $confirm, $format } = useGlobal()
    ...
    const cardTableConfig = ref<ICTCardTable>({
      ...
      columns: [
        ...
        { prop: 'createTime', label: '创建时间',formatter:(row:ITableItem)=>$format(row.createTime) }
      ],
      operations: [
        {
          label: '编辑',
    ...
    const refreshData = async () => {
      ...
      try {
        const res = await getMaintStandardTable(paramsObj)
    ...
    const handleStatusChange = (row?: ITableItem) => {
      console.log(row)
      //
    }
    ...
    const formateDevs = (row: ITableItem) => {
      console.log(row)
      return '设备1'
    }
    return {
