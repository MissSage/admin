      ...
      >
        ...
        <el-table-column label="判断">
          <template #default="scope">
            ...
            <div v-else class="selectText">
              {{ formateCondition(scope.row.condition) }}
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="条件关系">
          <template #default="scope">
            ...
            <div v-else class="selectText">
              {{ formateRelation(scope.row.relation) }}
            </div>
          </template>
        </el-table-column>
...
import {
  ...
  IPlanJob,
  IPlanTrigger,
  IPlanTriggerListItem
} from '@/common/types/maintenance/plan'
import SLDialog from '@/components/SLDialog/index.vue'

import { IStandard } from '@/common/types/maintenance/standard'
import { addMaintainPlan, getMaintainPlanDetail } from '@/api/maintenance/plan'
...
export default defineComponent({
  ...
  setup(props, ctx) {
    ...
    const { $messageError, $messageWarning, $messageSuccess, $confirm } = useGlobal()
    ...
    const setDefaultData = () => {
      getMaintainPlanDetail(props.currentId)
        .then(res => {
          console.log(res)
          if (res.status == 200) {
    ...
    const setBaseParams = (params: IPlan) => {
      ...
      if (activeName.value == GUDINGRIQIJIHUA) {
        ...
        if (dataForm.value.trigger) {
          ...
          dataForm.value.trigger.standardId = params.trigger?.standardId ||''
        }
      }
    ...
    const submit = async () => {
      ...
      try {
        const res = await addMaintainPlan(data as IPlan)
        if (res.status === 200) {
          $messageSuccess('操作成功')
          props.close()
    ...
    onMounted(async () => {
      const res = await getAllMaintStandards('')
      if (res.status == 200) {
