...
import type { ICTCardTable, ITableItem } from '@/common/types/repair/plan'
import { changeRepairePlanStatus, deleteRepairePlans, getRepairePlanTable } from '@/api/repair/plan'
import { QueryListParam } from '@/common/types/common'
import { CHUFAXINGJIHUA, GUDINGRIQIJIHUA, YUFANGXINGJIHUA } from '@/common/constans/repair'
export default defineComponent({
  ...
  setup() {
    ...
    const refreshData = async () => {
      ...
      try {
        const res = await getRepairePlanTable(paramsObj)
        console.log(res)
    ...
    const handleStatusChange = async (row: ITableItem) => {
      const res = await changeRepairePlanStatus(row.id)
      if (res.status == 200) {
        refreshData()
      }
    }
