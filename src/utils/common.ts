const base = {
  ...
  matchUrlIp (url: any, ip: any) { // url使用是否使用的当前ip
    if (!url || !ip) {
      return false
    }
    return url.indexOf(ip.replace('https://', '').replace('http://', '')) >= 0
  },
  getImgSrc (src: any, httpUrl: any) {
    if (this.isUrl(src)) {
      return src
    }
    if (httpUrl) {
  ...
  previewImg (src: any, httpUrl?: any) { // 图片预览，目前只支持单图片预览
    ...
    const $img1 = document.body.appendChild($div).querySelector('.vol-preview-img')
    // img.src = src;
    $img1 && $img1.setAttribute('src', src)
    $div.style.display = 'block'
  },
  // 下载文件 $element 标签, url完整url, fileName 文件名, header 以key/value传值
  // backGroundUrl 后台url，如果后台url直接从后台下载，其他全部通过点击a标签下载
  dowloadFile (url: any, fileName: any, header: any, backGroundUrl: any) {
  ...
  downloadImg (data: any) {
    ...
    xmlResquest.onload = function (callback: any) {
      if (this.status === 200) {
  ...
  convertTree (data: any, callback: any) {
    const treeIds: any[] = []
    const rootData: any[] = []
    data.forEach((x: any) => {
      // if (!x.children) {
      //   x.children = []
      // }
      if (!x.hidden && x.id !== undefined && x.id !== x.parentId && !data.some((s: any) => {
  ...
  getTreeAllParent (id: any, data: any) { // 获取某个节点的所有父节点信息2020.11.01
    const nodes: any[] = []
    ...
    const _child = data.find(x => { return x.id === id })
    if (!_child) {
      return []
    }
    nodes.push(_child)
    const _parentIds = [_child.parentId]
    for (let index = 0; index < _parentIds.length; index++) {
      const _node = data.find(x => { return x.id === _parentIds[index] && x.id !== x.parentId })
      if (!_node) {
        return nodes
      }
...
// 2020.06.01增加通用方法，将普通对象转换为tree结构
function getTree (id: any, node: any, data: any, callback: any, treeIds: any) {
